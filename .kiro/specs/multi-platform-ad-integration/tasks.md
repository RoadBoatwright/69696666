...
任务按依赖顺序组织：基础设施 → 领域核心层 → 平台适配器 → 获客主线应用服务（5步法获客闭环）→ 效果看板与业务指标 → 前端 → 高级投放能力全集 → LINE 渠道接入 → 集成。带 `*` 的为可选测试任务。

**渠道范围更新（本期）**：在 Meta / Google / TikTok 之外，本期新增 **LINE 广告渠道（LINE Ads）** 接入（见第六部分之三，任务 43）；LinkedIn 仍为第二期预留位。
...
- [x] 14. 实现 AI 辅助建广告引擎（组件 5，需求 9）
  - [x] 14.1 generateDraft 按画像生成多平台草案（合并旧知识库/行业报告/画像/草案；缺素材/画像维度返回缺失项；Gemini 凭据缺失降级）_Requirements: 9.2, 9.7, 9.8_
  - [x] 14.9 derivePersona 产品定位描述→AI 自动推导买家画像（来源标记）_Requirements: 9.1_
  - [x] 14.10 人工审核模式两级配置（全局默认+单商家覆盖）+ 全自动档自动确认/专家把关档置待确认 _Requirements: 9.3, 9.4_
  - [x] 14.2 canPublish 草案确认状态机 + confirm 驱动投放 _Requirements: 9.5, 9.6_
  - [x]* 14.3 属性测试 Property 18 草案确认状态机（待确认不得投放）_Requirements: 9.5, 9.6_
  - [x]* 14.4 属性测试 Property 19 获客方案缺失项完整反馈 _Requirements: 9.8_
  - [x] 14.5 投放优化建议与受限自动优化（MCP优先/官方API；上下限内应用；数据缺失不生成）_Requirements: 9.9-9.14_
  - [x] 14.11 以有效高意向商机为优化目标 + 相似扩展/低意向排除建议（预算优先分配给高意向占比高的受众/版位）_Requirements: 9.15, 9.16_
...
- [x] 21. 实现数据回传服务与审核同步（组件 12、13，需求 18、19、20）
  - [x] 21.1 指标拉取/归一化/computeRoi/单平台失败隔离 _Requirements: 18.1-18.7_
  - [x]* 21.2 属性测试 Property 32 ROI 与零花费 _Requirements: 18.2, 18.3_
  - [x]* 21.3 属性测试 Property 33 单平台失败隔离 _Requirements: 18.7_
  - [x] 21.4 转化追踪配置与事件关联/未匹配 _Requirements: 19.1-19.7_
  - [x]* 21.5 属性测试 Property 34 转化关联/未匹配 _Requirements: 19.5, 19.6_
  - [x] 21.6 审核状态同步归一化三态 + 变更通知 _Requirements: 20.1-20.5_

- [x] 22. 实现异步调度接线（BullMQ）
  - 令牌保活/状态扫描、指标15min拉取、审核15min轮询、线索回流后触发背调、背调失败重试、分级重算、自动路由、跟进失败重试
  - _Requirements: 2.4, 5.2, 18.5, 20.1, 15.2, 16.5, 17.3, 17.7_

### 第四部分：效果看板与业务结果指标

- [x] 23. 实现效果看板服务（组件 14，需求 21）
  - [x] 23.1 聚合看板 + validateRange + 按平台/时间范围 + 缺失维度隔离 _Requirements: 21.1-21.4_
  - [x]* 23.2 属性测试 Property 43 看板时间范围有效性与缺失隔离 _Requirements: 21.2, 21.3, 21.4_
  - [x] 23.2 5 个业务指标纯函数（询盘成本/联络率/意向占比/投放时长/客户资产）_Requirements: 21.5, 21.6, 21.7, 21.8, 21.9_
  - [x]* 23.3 属性测试 Property 44/45/46/47（询盘成本/联络率/占比/投放时长零除与边界）_Requirements: 21.5, 21.7, 21.8_
  - [x]* 23.4 属性测试 Property 48 客户资产停投不丢失 _Requirements: 21.9_
  - [x] 23.5 客户行业调查报告导出（维度缺失/AI凭据缺失标不可用）_Requirements: 21.11, 21.12_
  - [x] 23.6 商机清单视图 + 筛选/排序（等级/平台/跟进状态/时间，L4优先）_Requirements: 21.13_
  - [x] 23.7 单客户详情聚合（留资+质量标注+背调补全+等级轨迹+跟进剧本）_Requirements: 21.15_
  - [x] 23.8 商机清单导出 Excel/CSV（空集生成仅表头、越权拒绝）_Requirements: 21.14, 21.16, 21.17_
  - [x]* 23.9 属性测试 商机导出/详情 RBAC 数据隔离 _Requirements: 21.17_

- [x] 24. 检查点 - 效果看板与指标测试通过
...
### 第六部分之三：LINE 渠道接入（本期新增）

> LINE 广告平台为 LINE Ads（LINE Ads API / LINE Business），按现有平台接入模式逐层扩展；凭据占位符配置，未填入时按优雅降级标「不可用」。

- [ ] 43. LINE 渠道接入
  - [ ] 43.1 LINE API 能力盘点与 spec 增补（platform-api-capabilities.md 增加 LINE 条目，明确可用/不可用能力）
  - [ ] 43.2 统一模型扩展：`PlatformId` 增加 `'line'`；统一↔LINE 原生字段映射表与往返映射；LINE 不支持的维度标「不适用」_Requirements: 8.4, 8.5, 10.4_
  - [ ] 43.3 凭据管理扩展：PLATFORM_CREDENTIAL 增加 LINE 配置项（占位符），二态状态/加密/脱敏/可用性隔离复用现有机制 _Requirements: 1.1-1.8, 6.1-6.5_
  - [ ] 43.4 授权中心扩展：LINE 代客授权建立/撤销检测、令牌状态机与刷新接入 _Requirements: 5.1-5.7_
  - [ ] 43.5 LineAdapter 实现（真实 LINE Ads API）：三级结构 CRUD、素材上传、投放、指标拉取、审核状态；注册到适配器工厂 _Requirements: 8.4, 10.3, 13.1, 18.1, 20.1_
  - [ ] 43.6 全链路接通：草案生成、投放编排、优化引擎、指标/审核调度、看板覆盖 LINE；LINE 单平台失败不影响其他平台 _Requirements: 1.4, 13.1, 18.7_
  - [ ]* 43.7 测试：映射往返/可用性隔离/失败隔离属性测试扩展到 line；LINE 沙盒集成测试（凭据占位）_Requirements: 8.4, 1.4, 18.7_

### 第七部分：最终集成

- [ ] 41. 最终集成与端到端装配
  - [ ] 41.1 全模块装配与依赖注入接线（含 LINE 适配器与 LinkedIn 扩展位注册）_Requirements: 1.4, 8.9, 7.4_
...
- 第六部分高级投放能力（任务 28-40）与获客主线同属本期实现，充分利用各平台官方 API 高级能力；其属性测试在实现时按 design 补充。仅 LinkedIn（任务 40）为第二期平台扩展位预留。
- LINE 渠道（任务 43）为本期新增平台，接入模式与 Meta/Google/TikTok 同构；LINE 高级能力按官方 API 支持度经扩展网关探测，不支持的标「不可用」。
...
```json
{
  "waves": [
    ...
    { "id": 12.5, "tasks": ["43.1", "43.2", "43.3", "43.4", "43.5", "43.6", "43.7"] },
...
依赖关系说明：基础设施(1)→数据库(2)→凭据管理器(3)/授权中心(5)/权限(7)/统一模型(8)→平台适配器(10)→广告计划(12)/素材(13)→AI建广告(14)/线索(16)/数据回传(21)→背调(17)→分级(18)→跟进(19)→效果看板(23)→前端(25-27)；获客主线(1-27)、高级投放能力全集(28-40)与 LINE 渠道接入(43)均本期实现，最后做集成(41-42)。
